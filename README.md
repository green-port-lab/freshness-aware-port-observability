...
infrastructure: fixed water-quality buoys, shore-based LoRaWAN gateways, port service vessels,
unmanned aerial vehicles, and workers carrying radios and ultra-wideband badges. Classical and
intermittent-observation formulations assess state reconstructability under a given
measurement model or observation probability, but they do not model the time-respecting
paths along which data reach the decision point or how fresh the data are at decision time. During an
...
| Result | Value |
|---|---|
| Mean score over the accident interval (60–115 min) | 0.192 (A) / 0.280 (B) / 0.398 (C) |
| Total duration of Δ-unobservability, control run vs main scenario | 60 / 60 min for A and B; 25 / 45 min for C |
| Overestimate by the time-aggregated static graph | 0.084 on average, up to 0.215 at 100 min |
...
| Runtime per step for a 250-node network | 1119 ms, i.e. 0.37% of the 5-min time step |

The time-aggregated static graph overstates observability because of **observer islands**:
...
model/            the model and the computational experiments
  model2.py         directed links, latency, earliest arrival, score, criticality
  experiments.py    regenerates every published table and check
  counterexample.py stationary network where the static graph overstates the score
...
python figures/sources/figs.py      # graph snapshots and score dynamics
...
CPython 3.12.3 on an x86-64 Linux host, identified in the virtualized environment as
Intel Xeon Processor @ 2.10 GHz. No parallelization, no optimization of data structures.

## The scenario

An accident during a storm at a petroleum and vegetable oil terminal. In the manuscript the parameters are
grouped by origin into technical, operational, and assumed, and the assumed ones are
...
**Participants (15).** Control center CC and shore-based LoRaWAN gateways R1, R2 (wired to CC); buoys
...
terminal section is de-energized, so gateway R1 goes down; buoy S1 is torn from its
