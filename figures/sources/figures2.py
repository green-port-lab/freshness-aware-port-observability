...
comma = FuncFormatter(lambda v, p: f"{v:.1f}")
...
for ax, (key, title) in zip(axs, titles.items()):
    ...
    ax.plot(xs, [num(r["mean_A"]) for r in g], marker="o", ms=4.5, lw=1.7, color=COL["A"], label="configuration A")
    ax.plot(xs, [num(r["mean_C"]) for r in g], marker="o", ms=4.5, lw=1.7, color=COL["C"], label="configuration C")
    ax2 = ax.twinx()
    ...
    ax2.tick_params(axis="y", labelsize=8, colors="#6b7f95")
    if ax is axs[-1]: ax2.set_ylabel("unobservability duration, C, min", fontsize=8, color="#6b7f95")
...
axs[0].set_ylabel("$\\bar{O}$, accident interval")
from matplotlib.patches import Patch
h, l = axs[0].get_legend_handles_labels()
h.append(Patch(facecolor="#bcccdc", alpha=.55, edgecolor="none"))
l.append("unobservability duration, C (right axis)")
fig.legend(h, l, fontsize=8, frameon=False, loc="lower center", ncol=3, bbox_to_anchor=(0.5, -0.01))
plt.tight_layout(w_pad=1.6, rect=[0, 0.07, 1, 1])
plt.savefig(OUT + "/fig_sensitivity.png", dpi=600, bbox_inches="tight")
plt.close()
...
ax.loglog(n, ms, marker="o", ms=6, lw=1.8, color=NAVY, label="computation time per step")
for x, y, e in zip(n, ms, ed):
    ax.annotate((f"{e:,}" if e >= 10000 else f"{e}") + " edges", (x, y), textcoords="offset points", xytext=(0, 9), ha="center",
...
plt.tight_layout()
plt.savefig(OUT + "/fig_scalability.png", dpi=600, bbox_inches="tight")
plt.close()
...
for i, v in enumerate(vals):
    ax.text(v + 0.0016, i, f"{v:.3f}", va="center", fontsize=7.6, color="#333")
ax.set_xlabel("criticality $\\chi(v)$")
...
ax.legend(handles, ["buoy", "shore-based gateway", "port service vessel",
                    "tanker", "UAV", "port worker"], fontsize=7.6, frameon=False,
          loc="lower right")
plt.tight_layout()
plt.savefig(OUT + "/fig_criticality.png", dpi=600, bbox_inches="tight")
plt.close()
print("ok")
